...
from backend.models import (
    IntakeResult, PolicyResult, ReviewDecision,
...
@pytest.mark.parametrize("inquiry,intake_kwargs,policy_kwargs", [
    (
...
async def test_review_escalates_sensitive_cases(inquiry, intake_kwargs, policy_kwargs):
    intake = make_intake(**intake_kwargs)
    policy = make_policy(**policy_kwargs)

    with review_agent.override(model=EVAL_MODEL):
        result = await run_review(inquiry=inquiry, intake=intake, policy=policy)
